...
for u in range(U):
    ...
    kappa_nn = calculate_kappa(Y_train, predict_nn, plus = plus)
    p1_nn[u] = get_dnn_p_val(mse_train_nn, mse_test_nn, n_train, n_test, kappa_nn)
    ...
    kappa_dnn = calculate_kappa(Y_train, predict_dnn, plus = plus)
    p1_dnn[u] = get_dnn_p_val(mse_train_dnn, mse_test_dnn, n_train, n_test, kappa_dnn)
    ...
    kappa_dnn2 = calculate_kappa(Y_train, predict_dnn2, plus = plus)
    p1_dnn2[u] = get_dnn_p_val(mse_train_dnn2, mse_test_dnn2, n_train, n_test, kappa_dnn2)
...
p_val_nn_hommel = p_val_combine(p_nn, method = 'hommel')
p_val_dnn_hommel = p_val_combine(p_dnn, method = 'hommel')
p_val_dnn2_hommel = p_val_combine(p_dnn, method = 'hommel')

result0 = [p_val_nn_hommel, p_val_dnn_hommel, p_val_dnn2_hommel]

p_val_nn_hommel = p_val_combine(p1_nn, method = 'hommel')
p_val_dnn_hommel = p_val_combine(p1_dnn, method = 'hommel')
p_val_dnn2_hommel = p_val_combine(p1_dnn, method = 'hommel')


result1 = [p_val_nn_hommel, p_val_dnn_hommel, p_val_dnn2_hommel]

print("p_nn: " + str(p_nn))
print(p_val_nn_hommel)
import csv
title = ['p_nn', 'p_dnn', 'p_dnn2']
filename = f"result_tag0_{max_hidden_unit}_{nN}_hommel.csv"
import os
# writing to csv file
...
filename1 = f"result_tag1_{max_hidden_unit}_{nN}_hommel.csv"
